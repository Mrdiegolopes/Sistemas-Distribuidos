...
class Cliente:
    ...
    def listar_dispositivos(self):
        ...
        if resposta and resposta.response_type == cliente_gateway_pb2.RespostaGateway.LISTAR_DISPOSITIVOS:
            ...
            print("Falha ao obter dispositivos.")
    ...
    def enviar_comando(self, id_dispositivo, tipo_comando, valor=None):
        ...
        comando.id_despositivo = id_dispositivo
        comando.Comando_type = tipo_comando
        if valor is not None:
            comando.value = valor

        print(f"Enviando comando para {id_dispositivo}: {cliente_gateway_pb2.Comando.TipoComando.Name(tipo_comando)}")
        self.enviar_mensagem(requisicao)

        resposta = self.receber_mensagem()
        if resposta and resposta.response_type == cliente_gateway_pb2.RespostaGateway.ENVIAR_COMANDO:
            print(f"Comando para {id_dispositivo}: {'Sucesso' if resposta.sucesso else 'Falha'} - {resposta.message}")
        elif resposta:
            print(f"Resposta inesperada: {resposta.response_type} - {resposta.message}")
        else:
            print("Falha ao obter resposta.")
